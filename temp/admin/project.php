...
if (isset($_GET['h'])) {
    ...
    $hide = mysqli_query($conn, "update project set status='NO' where project_id='$gid'");
    if ($hide) {
        echo "<script>alert('Photo Hidden from home page successfully!'); window.location.href='project.php'</script>";
    } else {
...
if (isset($_GET['p'])) {
    ...
    $hide = mysqli_query($conn, "update project set status='YES' where project_id='$gids'");
    if ($hide) {
        echo "<script>alert('Project posted on home page successfully!'); window.location.href='project.php'</script>";
    } else {
        echo "<script>alert('Operations Failed, Please try again after some minutes!')</script>";
    }
}
...
if (isset($_GET['d'])) {
    ...
    $img_url = mysqli_fetch_array(mysqli_query($conn, "select url from project where project_id='$dids'"));
    $hide = mysqli_query($conn, "delete from project where project_id='$dids'");
    if ($hide) {
        $image = $img_url["url"];
        unlink("media/gallery/$image");
        echo "<script>alert('photo deleted successfully!'); window.location.href='project.php'</script>";
    } else {
        echo "<script>alert('Operations Failed, Please try again after some minutes!')</script>";
    }
}
...
if (isset($_POST["upload"])) {
    ...
    if ($pic_name != "") {
        ...
        if ($img_url != "") {
            $insert_test = mysqli_query($conn, "insert into project values ('$galid','$img_url','$comment','$date','$status')");
            if ($insert_test) {
...
<html>
    <head>
        <meta charset="UTF-8">
        <title>Admin | Project Photo Upload</title>
                        ...
                        <div class="col-md-3 col-sm-6 col-xs-12">
                            <div class="info-box">
                                ...
                                <div class="info-box-content">
                                    <?php
                                    $num = mysqli_num_rows(mysqli_query($conn, "select * from project"));
                                    ?>
                        ...
                        <div class="col-md-12">
                            <div class="box">
                                <div class="box-header with-border">
                                    <h3 class="box-title">Upload Project Photo</h3>
                                ...
                                <div class="nav-tabs-custom">
                                    ...
                                    <ul class="nav nav-tabs pull-right">
                                        ...
                                        <li class="active"><a href="#sales-chart" data-toggle="tab"> PROJECT PHOTOS</a></li>
                                    </ul>
                                        ...
                                        <div class="chart tab-pane active" id="sales-chart" style="position: relative; height: 100%; overflow-x: scroll;">
                                            <div class="box-body">
                                                ...
                                                <table id="example1" class="table table-bordered table-hover">
                                                    ...
                                                    <tbody>
                                                        <?php
                                                        $result = mysqli_query($conn, "select * from project order by date desc");
                                                        $posted_testimonies = mysqli_num_rows(mysqli_query($conn, "select * from project where status='YES'"));
                                                        $a = 1;
                                                            ...
                                                            <tr>
                                                                ...
                                                                <td>
                                                                    ...
                                                                    if ($posted_testimonies < 6) {
                                                                        ?>
                                                                        <?php if ($status == "NO") { ?> 
                                                                            <a href='project.php?p=<?php echo base64_encode($pic_id); ?>'>POST</a>
                                                                        <?php } else { ?>
                                                                            <a href='project.php?h=<?php echo base64_encode($pic_id); ?>'>HIDE</a>
                                                                            <?php
                                                                        }
                                                                    } else {
                                                                        ?>
                                                                ...
                                                                <td><a onclick="return confirm('press ok to delete photo')" href="project.php?d=<?php echo base64_encode($pic_id); ?>">DELETE</a></td>
                                                            </tr>
                                                            <?php
                                                            $a++;
                                                        }
