...
$rw3 = mysqli_fetch_array(mysqli_query($conn, "select * from customer where cus_id ='$id' ")); 
$venture = $rw3['venture'];
                            ...
                            $message = 'Dear esteem customer your order as been processed and will be enroute to your location. Thank You for your patronage!';
                            $senderid = 'ETransaction';
                            $recipients = $rw3['phone'];
...
//Or to validate by calling the validate_sendsms function
                            if (validate_sendsms($response) == TRUE) {
                              echo "<script> alert('Message has been sent to customer successfully!'); window.location.href='vieworder.php';</script>";
                            }else{
                              echo "<script> alert('Message has been sent to customer successfully!'); window.location.href='vieworder.php';</script>";

                                //echo "<script> alert('Message not sent, Error occured!'); window.location.href='vieworder.php';</script>";
                            }
                            //end of token api
                            ...
                            <div class="box">
                                ...
                                <div class="box-body" style="overflow-x: scroll;">
                                    <table id="example1" class="table table-bordered table-hover">
                                        ...
                                        <tbody>
                                            <?php
                                            $result = mysqli_query($conn, "select * from customer_order where status='pending'");
                                            $a = 1;
                                            while ($row = mysqli_fetch_array($result)) {
                                                ...
                                                <tr>
                                                    ...
                                                    <td>
                                                        <a href="vieworder.php?update_id=<?php echo $row['cus_id']; ?>">ACCEPT</a> |
                                                        <a href="viewprintproduct.php?m=<?php echo base64_encode($cus_name); ?>">PRINT</a> |<br>
                                                        <a href="vieworder.php?info=<?php echo $get_cus_id; ?>">INFO CUSTOMER</a> |
